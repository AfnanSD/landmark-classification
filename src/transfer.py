...
def get_model_transfer_learning(model_name="resnet18", n_classes=50):
    ...
    num_ftrs  = model_transfer.fc.in_features # <- assuming it's resnet18 model and it's final layer is called fc

    # 2. Create a new linear layer with the appropriate number of inputs and
    #    outputs
    model_transfer.fc  = torch.nn.Linear(in_features=num_ftrs, out_features=n_classes, bias=True)
